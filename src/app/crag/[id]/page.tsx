...
export default function CragDetailPage({ params }: PageProps) {
  const { id } = use(params)
  ...
  if (!crag) {
    return (
      <div className="flex items-center justify-center h-screen">
        <p className="text-[var(--m3-on-surface-variant)]">岩场不存在</p>
      </div>
    )
  }

  // 生成封面图 URL
  const images = [1, 2].map((n) => `${COS_BASE_URL}/CragSurface/${id}/${n}.jpg`)
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-[var(--m3-surface)]">
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-[var(--m3-surface)] to-transparent">
        <Button
          onClick={() => router.push(`/route?crag=${id}`)}
